...
class DRaFT_DDPM(DDPM):
    ...
    def compute_perceptual_loss_reward(
        ...
        """
        self.reward_encoder.eval()
        gen_features = self.reward_encoder(generated)
        with torch.no_grad():
            target_features = self.reward_encoder(target)
    ...
    def compute_cosine_similarity_reward(
        ...
        with torch.no_grad():
            target_embedding = self.reward_encoder(target)
    ...
    def ddim_sample_differentiable(
        ...
        out = self.p_mean_variance(
            x, t, clip_denoised=clip_denoised, 
        ...
        mean_pred = pred_x0 * a_prev.sqrt() + eps * (1 - a_prev - sigma**2).sqrt()
