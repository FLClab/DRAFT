...
parser.add_argument("--subsamples", nargs="+", type=int, default=[50, 100, 300, 500, 1000, 2000, 3000, None])
...
METRIC_NAMES = ["mse", "ssim", "ms_ssim", "fourier_ncc_low", "fourier_ncc_high", "stedfm", "lpips"]
...
def inference(
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc=f"... Inferring {model_type}_{subsample}-sample samples ..."):
            ...
            mse, ssim, ms_ssim, fourier_ncc_low, fourier_ncc_high, stedfm_sim, lpips = sample_metrics["mse"], sample_metrics["ssim"], sample_metrics["ms_ssim"], sample_metrics["fourier_ncc_low"], sample_metrics["fourier_ncc_high"], sample_metrics["stedfm"], sample_metrics["lpips"]
...
def save_stats(data: dict, metric: str,subsample: int, save_dir: str):
    os.makedirs(os.path.join(save_dir, "stats"), exist_ok=True)
    keys = list(data.keys())
    pairs = combinations(keys, 2)
    with open(os.path.join(save_dir, "stats", f"{metric}_stats_{subsample}_sample.txt"), "w") as f:
        for (p1, p2) in pairs:
            data_p1 = [float(item) for item in data[p1]]
            data_p2 = [float(item) for item in data[p2]]
...
def analyze_results(pix2pix_results: dict, ddpm_results: dict, draft_results: dict, save_dir: str):
    ...
    resolution_mean = np.mean(real_resolution)
    quality_mean = np.mean(real_quality)
    resolution_std = np.std(real_resolution, ddof=1) / np.sqrt(len(real_resolution))
    quality_std = np.std(real_quality, ddof=1) / np.sqrt(len(real_quality))
    print(f"Resolution mean: {resolution_mean:.4f}, Resolution std: {resolution_std:.4f}")
    print(f"Quality mean: {quality_mean:.4f}, Quality std: {quality_std:.4f}")
    for metric_key in METRIC_NAMES:
        ...
        for subsample in tqdm(ddpm_results.keys(), desc="... Aggregating results ..."):
            ...
            try:
                ...
                save_stats(data={"pix2pix": pix2pix_data, "ddpm": ddpm_data, "draft": draft_data}, metric=metric_key, subsample=subsample, save_dir=save_dir)
            except:
                continue 
        ...
        n = ddpm_metric_avgs.size 
        ddpm_metric_stds = np.std(ddpm_metric_avgs, ddof=1) / np.sqrt(n)
        # ddpm_metric_stds = np.array(ddpm_metric_stds)
        draft_metric_avgs = np.array(draft_metric_avgs)
        n = draft_metric_avgs.size 
        draft_metric_stds = np.std(draft_metric_avgs, ddof=1) / np.sqrt(n)
        pix2pix_metric_avgs = np.array(pix2pix_metric_avgs)
        n = pix2pix_metric_avgs.size 
        pix2pix_metric_stds = np.std(pix2pix_metric_avgs, ddof=1) / np.sqrt(n)
        ...
        if metric_key == "resolution":
            ax.axhline(resolution_mean, color="grey", linestyle="--", label="Real data")
            ax.fill_between(x, resolution_mean - resolution_std, resolution_mean + resolution_std, color="grey", alpha=0.2) 
        if metric_key == "quality":
            ax.axhline(quality_mean, color="grey", linestyle="--", label="Real data")
            ax.fill_between(x, quality_mean - quality_std, quality_mean + quality_std, color="grey", alpha=0.2)
        ...
        fig.savefig(os.path.join(save_dir, f"{metric_key}.pdf"), dpi=900, transparent=True)
        plt.close(fig)
...
def compute_real_data_metrics(dataset: DendriticFActinDataset, quality_net: nn.Module):
    ...
    return np.array(resolutions), np.array(qualities)
